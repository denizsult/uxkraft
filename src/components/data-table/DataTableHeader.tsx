  ...
  return (
    <TableHeader>
      {table.getHeaderGroups().map((headerGroup) => (
        <TableRow
          key={headerGroup.id}
          className="bg-[#f6f3f3] border-b border-[#d8d8d8] hover:bg-[#f6f3f3]"
        >
          {headerGroup.headers.map((header, index) => {
            ...
            return (
              <TableHead
                key={header.id}
                className={`h-10 px-3 py-2.5 ${meta?.headerClassName || ""} ${
                  !hasBorderInMeta ? "border-r border-[#d8d8d8]" : ""
                } ${
                  isLast ? "last:border-r-0" : ""
                }  font-medium text-muted-gray text-xs`}
              >
